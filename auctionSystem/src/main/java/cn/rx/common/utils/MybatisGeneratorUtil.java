...
public class MybatisGeneratorUtil {
	...
	// generatorConfig模板路径
	private static String generatorConfig_vm = "/mybatis/generator/generatorConfig.vm";
	// Service模板路径
	private static String service_vm = "/mybatis/generator/Service.vm";
	// ServiceMock模板路径
	private static String serviceMock_vm = "/mybatis/generator/ServiceMock.vm";
	// ServiceImpl模板路径
	private static String serviceImpl_vm = "/mybatis/generator/ServiceImpl.vm";
	...
	public static void generator(
		...
		try {
			...
			for (Map map : result) {
				...
				table.setTable_name((String)map.get("TABLE_NAME"));
				table.setModel_name(lineToHump(ObjectUtils.toString(map.get("TABLE_NAME"))));
				table.setDo_name(lineToHump(ObjectUtils.toString(map.get("TABLE_NAME")))+"DO");
				if (lastInsertIdTables!=null) {
					table.setLAST_INSERT_ID_TABLES(lastInsertIdTables.get(table.getTable_name()));
				}
				if (tableAlias!=null) {
					table.setAlias(tableAlias.get(table.getTable_name()));
				}
				tables.add(table);
			}
			jdbcUtil.release();
			...
			deleteDir(new File(targetProjectSqlMap + "/src/main/resources/mybatis/mapper"));
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		/*	File serviceMockFile = new File(serviceMock);
			...
			if (!serviceImplFile.exists()) {
				...
				context.put("DOName", DOName);
				context.put("model", model);
				context.put("mapper", StringUtil.toLowerCaseFirstOne(DOName));
				context.put("ctime", ctime);
